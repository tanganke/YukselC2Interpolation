...
namespace glm {
/**
 * @brief Calculates the L2 norm (Euclidean norm) of a 2D vector.
 *
 * @tparam T The type of the vector's components.
 * @tparam Q The qualifier of the vector.
...
template <typename VectorType> class QuadraticBezier {
public:
...
public:
  ...
  /**
   * @brief Construct a new Quadratic Bezier object.
   *
   * @param x The first control point.
   * @param y The second control point.
   * @param z The third control point.
   */
  QuadraticBezier(const VectorType &x, const VectorType &y, const VectorType &z)
      : _x{x}, _y{y}, _z{z} {}

  /**
   * @brief Evaluate the Bezier curve at a given parameter t.
   *
   * @param t The parameter at which to evaluate the curve.
   * @return VectorType The point on the curve at parameter t.
   */
...
template <typename VectorType> class QuadraticBezierInterpolation {
public:
  using value_type = typename VectorType::value_type;
...
public:
  ...
  QuadraticBezierInterpolation(const VectorType &p0, const VectorType &p1,
                               const VectorType &p2) {
    static auto square = [](value_type x) { return x * x; };
    value_type ti = 0.5;
    ...
    default:
      ...
      gsl_poly_complex_solve_cubic(b / a, c / a, d / a, complex, complex + 1,
                                   complex + 2);
      for (int i = 0; i < 3; i++) {
        if (complex[i].dat[0] < 1 && complex[i].dat[0] > 0 &&
            complex[i].dat[1] < 0.01) {
    ...
    VectorType b1 =
        (p1 - square(1 - ti) * p0 - square(ti) * p2) / (2 * (1 - ti) * ti);
    _bezier = QuadraticBezier<VectorType>(p0, b1, p2);
  }

  VectorType operator()(value_type t) { return _bezier(t); }
};

template <typename VectorType, typename InterpF> class YukselC2Interpolation {
public:
  using value_type = typename VectorType::value_type;
...
private:
  ...
  VectorType blend(int n1, int n2, value_type theta) {
    return std::cos(theta) * std::cos(theta) *
               scaled_call(n1, theta + std::numbers::pi_v<value_type> / 2) +
           std::sin(theta) * std::sin(theta) * scaled_call(n2, theta);
  }

public:
  YukselC2Interpolation(std::span<VectorType> points)
  ...
  /**
   * @brief Evaluate the Bezier curve at a given segment and parameter.
   *   0<t<pi/2
   * @param n The segment of the curve to evaluate.
   * @param theta The parameter at which to evaluate the curve.
   * @return VectorType The point on the curve at the given segment and
   * parameter.
   */
